...
fun buildSingBoxOutboundHysteriaBean(bean: HysteriaBean): SingBoxOptions.SingBoxOption {
    val relayHost = if (bean.turnEnabled == true) LOCALHOST else bean.serverAddress
    val relayPort = if (bean.turnEnabled == true) (bean.turnRelayPort ?: 9000) else null

    return when (bean.protocolVersion) {
        1 -> SingBoxOptions.Outbound_HysteriaOptions().apply {
            type = "hysteria"
            server = relayHost
            val port = relayPort ?: bean.serverPorts.toIntOrNull()
            if (port != null) {
                server_port = port
            } else {
