...
fun buildSingBoxOutboundWireguardBean(bean: WireGuardBean): SingBoxOptions.Outbound_WireGuardOptions {
    val relayHost = if (bean.turnEnabled == true) "127.0.0.1" else bean.serverAddress
    val relayPort = if (bean.turnEnabled == true) (bean.turnRelayPort ?: 9000) else bean.serverPort
