...
object DataStore : OnPreferenceDataStoreChangeListener {
    ...
    var turnEnabled by profileCacheStore.boolean(Key.TURN_ENABLED)
    var turnServer by profileCacheStore.string(Key.TURN_SERVER)
    var turnPort by profileCacheStore.stringToInt(Key.TURN_PORT) { 3478 }
    var turnPeer by profileCacheStore.string(Key.TURN_PEER)
    var turnAuthLink by profileCacheStore.string(Key.TURN_AUTH_LINK)
    var turnSource by profileCacheStore.string(Key.TURN_SOURCE) { "vk" }
    var turnUseUdp by profileCacheStore.boolean(Key.TURN_USE_UDP)
    var turnRelayPort by profileCacheStore.stringToInt(Key.TURN_RELAY_PORT) { 9000 }
